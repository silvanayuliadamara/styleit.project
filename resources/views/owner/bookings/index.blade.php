@extends('layouts.owner', ['title' => 'Daftar Booking & Transaksi — LYB'])
...
@section('owner_content')
    ...
    <header class="lyb-admin-page-header">
        <div>
            ...
            <p>Kelola semua booking MUA dan transaksi yang masuk.</p>
        </div>
        <div style="min-width: 260px;">
            <form action="{{ route('owner.bookings.index') }}" method="GET" class="position-relative">
    ...
    <div class="d-flex gap-2 mb-3 flex-wrap align-items-center">
        ...
        <a href="{{ route('owner.bookings.index', ['payment_status' => 'dp_diupload']) }}" class="btn btn-sm rounded-pill px-3 {{ request('payment_status') === 'dp_diupload' ? 'btn-dark' : 'btn-outline-dark' }}" style="font-size: 12px; font-weight: 600;">
            Perlu Konfirmasi
        </a>
        <a href="{{ route('owner.bookings.index', ['payment_status' => 'dp_diterima']) }}" class="btn btn-sm rounded-pill px-3 {{ request('payment_status') === 'dp_diterima' ? 'btn-dark' : 'btn-outline-dark' }}" style="font-size: 12px; font-weight: 600;">
            DP Dibayar
        </a>
                            ...
                            <tr>
                                ...
                                <td class="text-end text-nowrap">
                                    <div class="d-inline-flex gap-2">
                                        <a href="{{ route('owner.bookings.invoice', $booking) }}" class="lyb-admin-action-btn" style="background-color: #fbf8f1; border: 1px solid #eadfd6; color: #5c430e;">
                                            <i class="bi bi-receipt"></i> Invoice
                                        </a>
                                        <a href="{{ route('owner.bookings.show', $booking) }}" class="lyb-admin-action-btn">

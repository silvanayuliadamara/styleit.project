@extends('layouts.owner', ['title' => 'Tambah Addon & Tambahan — LYB'])
...
@section('owner_content')
    ...
    <header class="lyb-admin-page-header">
        <div>
            <h2>Tambah Addon & Tambahan</h2>
            <p>Buat item/layanan tambahan baru untuk memperluas pilihan transaksi customer.</p>
        </div>
    </header>

    <section class="lyb-admin-section">
        <form action="{{ route('owner.addons.store') }}" method="POST">
            @csrf
                    ...
                    <div class="card p-4 mb-4" style="border-radius: 18px; border: 1px solid #eadfd6; background: #fff;">
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="Contoh: Makeup Keluarga, Melati Pengantin" required style="border-radius: 10px; border-color: #eadfd6;">
                                @error('name')
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <input type="number" name="price" id="price" class="form-control @error('price') is-invalid @enderror" value="{{ old('price') }}" placeholder="350000" required style="border-radius: 10px; border-color: #eadfd6;">
                                @error('price')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" rows="3" placeholder="Tulis catatan ringkas tentang tambahan ini..." style="border-radius: 10px; border-color: #eadfd6;">{{ old('description') }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                <div class="form-check form-switch py-1">
                                    <input class="form-check-input" type="checkbox" name="is_pihak_lain" id="is_pihak_lain" value="1">
                            ...
                            <div class="col-12 d-none" id="biaya-vendor-container">
                                <label for="biaya_pihak_lain" class="form-label fw-bold small text-muted">Biaya Pihak Ketiga (COGS Vendor) (Rp)</label>
                                <input type="number" name="biaya_pihak_lain" id="biaya_pihak_lain" class="form-control" value="0" placeholder="Biaya modal ke vendor" style="border-radius: 10px; border-color: #eadfd6;">
                            </div>
                        </div>
                    </div>
                </div>
